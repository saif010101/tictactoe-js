...
function startGame() {
  ...
  gameBody.classList.remove("bg-red-500");
...
function gameTie() {
  // update info bar
  infoBar.textContent = "Match Tied!";

  // update game body color
  gameBody.classList.add("bg-red-500");

  // update button color and text
  startGameBtn.classList.add("bg-red-600");
  startGameBtn.classList.add(
    "hover:text-white",
    "hover:font-bold",
    "hover:scale-105"
...
function checkWinner(gridMatrix) {
  ...
  for (let i = 0; i < 3; i++) {
    if (
      gridMatrix[0][i] === gridMatrix[1][i] &&
      gridMatrix[1][i] === gridMatrix[2][i]
    )
  ...
  for (let i = 0; i < 3; i++) {
    if (
      gridMatrix[i][0] === gridMatrix[i][1] &&
      gridMatrix[i][1] === gridMatrix[i][2]
    )
...
function markGrid(event) {
  ...
  if (turn === 1 && currentGrid.innerHTML === "" && gameStarted) {
  ...
  } else if (turn === 2 && currentGrid.innerHTML === "" && gameStarted) {
  ...
  if (mark != "") endGame(mark);
  else if (markedGridsCount === 9) gameTie();
}
